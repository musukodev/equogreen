            ...
            <form action="{{ route('fastexcel.import') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl border border-gray-400 p-6 shadow-sm flex flex-col transition-all duration-300 hover:scale-[1.005] hover:shadow-md mb-8 mt-6">
                ...
                <div class="space-y-3 w-full">
                    ...
                    @if($hasUploaded && !$isEditing)
                        ...
                        <div class="bg-gray-50 rounded-xl border border-gray-300 p-5 mt-2 transition-all duration-300">
                            <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
                                <div>
                                    <h2 class="text-[16px] font-bold text-gray-800 mb-0.5">Status Pengiriman</h2>
                                    <p class="text-[13px] text-gray-500">File quotation telah berhasil diunggah.</p>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    <button type="button" wire:click="$set('isEditing', true)" class="flex items-center gap-2 bg-white text-blue-700 hover:bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-lg text-[13px] font-bold transition shadow-sm">
                                        <i class="fa-solid fa-pen-to-square"></i> Edit
                                    </button>
                                    <button type="button" wire:click="deleteQuotation" wire:confirm="Apakah Anda yakin ingin menghapus quotation ini?" class="flex items-center gap-2 bg-white text-red-700 hover:bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg text-[13px] font-bold transition shadow-sm">
                            ...
                            <div class="bg-white border border-gray-200 rounded-lg p-3 flex items-center justify-between shadow-sm">
                                <div class="flex items-center gap-3 overflow-hidden w-full">
                                    <div class="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0">
                                        <i class="fa-solid fa-file-excel text-blue-600 text-lg"></i>
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        <p class="text-[12px] text-gray-500 font-medium mb-0.5">Waktu Unggah: {{ $lastModified ?? '-' }}</p>
                                        <p class="text-[14px] font-bold text-[#1e40ff] truncate w-full">
                    ...
                    @else
                        ...
                        <!-- Tombol submit -->
                        <button type="submit" class="w-full bg-[#1e40ff] text-white text-center text-[15px] py-3.5 rounded-lg hover:bg-blue-700 transition relative font-bold">
                            {{ $hasUploaded ? 'Upload Ulang (Ubah)' : 'Kirim' }}
                        </button>
                        
                        @if($isEditing)
                        <button type="button" wire:click="$set('isEditing', false)" class="w-full bg-gray-200 text-gray-800 text-center text-[15px] py-3.5 rounded-lg hover:bg-gray-300 transition relative font-bold">
                            Batal Edit
                        </button>
                        @endif
                    @endif
